...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None,  log_path=None, theme='theme_default'):
        """
            生成测试报告,并放在当前运行路径下
        ...
        self.report_dir = os.path.abspath(log_path)
        os.makedirs(self.report_dir, exist_ok=True)
        self.suites.run(result=self)
        self.stopTestRun(self.title)
        self.output_report(theme)
        text = '\n选品系统接口自动化测试已全部完成, 可打开 {} 查看报告'.format(os.path.join(self.report_dir,self.filename))
        print(text)
